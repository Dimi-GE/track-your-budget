function initEarningsHeatmap() {
    ...
    const yearUpBtn   = document.getElementById('earnings-year-up');
    const yearDownBtn = document.getElementById('earnings-year-down');
    const yearSlots   = document.getElementById('earnings-year-slots');
    const qContainer  = document.getElementById('earnings-quarter-btns');
    const grid        = document.getElementById('earnings-heatmap-grid');
    const monthLabels = document.getElementById('earnings-month-labels');
    ...
    function renderGrid() {
        ...
        for (let i = 0; i < totalCells; i++) {
            ...
            cell.className = 'heatmap-cell earnings-cell';
            if (i < startDow) {
    ...
    function render(notifySync = true) {
        ...
        if (notifySync && typeof onHeatmapPeriodChange === 'function') {
            onHeatmapPeriodChange('earnings', year, quarter);
        }
    }

    window.setEarningsPeriod = function(y, q) {
        year    = y;
        quarter = q;
        render(false);
    };
